...
pub fn archive_device_windows(
    ...
    if is_cd {
        ...
        let sector_modes =
            rip_cd_to_bin(drive.0, &tracks, &bin_path, DEFAULT_RETRIES, total_sectors, &sink)?;
...
fn get_volume_label(letter: char) -> Option<String> {
    ...
    let label = OsString::from_wide(&buf[..len]).to_string_lossy().to_string();
